// ============================================
// Glow Studio by Sofia — Staff Seed Script (TypeScript)
// ============================================
...
export async function seedStaff(prismaInstance?: PrismaClient) {
  const prisma = prismaInstance || new PrismaClient();
  const seededStaff = [];

  for (const s of staffSeedData) {
    if (!s.email) continue;
    ...
    console.log(`✅ Staff member seeded: ${staff.name} (${staff.email})`);
  }

  console.log(`✨ Total ${seededStaff.length} staff members seeded successfully!`);
  return seededStaff;
}
